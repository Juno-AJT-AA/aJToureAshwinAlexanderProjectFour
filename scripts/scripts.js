...
radMovieQuiz.displayQuiz = () => {
  ...
  if (arrayMovies.length == 4) {
    ...
    $(".movieOptionOne input[type=image]").attr("alt", "Is this a 90s movie?: " + arrayMovies[0].title);

    $(".movieOptionTwo input[type=image]").attr("src", radMovieQuiz.baseImageURL.concat(arrayMovies[1].poster_path));
    $(".movieOptionTwo input[type=image]").attr("alt", "Is this a 90s movie?: " + arrayMovies[1].title);

    $(".movieOptionThree input[type=image]").attr("src", radMovieQuiz.baseImageURL.concat(arrayMovies[2].poster_path));
    $(".movieOptionThree input[type=image]").attr("alt", "Is this a 90s movie?: " + arrayMovies[2].title);

    $(".movieOptionFour input[type=image]").attr("src", radMovieQuiz.baseImageURL.concat(arrayMovies[3].poster_path));
    $(".movieOptionFour input[type=image]").attr("alt", "Is this a 90s movie?: " + arrayMovies[3].title);
  }
};

//highlight the correct option and also, the incorrect answer selected if any
radMovieQuiz.styleRightWrong = () => {
  //first style the one in focus
  $(`.movieOption:nth-of-type(${radMovieQuiz.selectedMovieIndex}) div`)
    .text(
      "❌ Released: " +
      radMovieQuiz.finalFour[
        radMovieQuiz.selectedMovieIndex - 1
      ].release_date.split("-")[0]
    )
    .addClass("focusAnswer");

  //next style the image that is correct
  let answerIndex = radMovieQuiz.getAnswerIndex();
  let nthOfTypeValue = answerIndex + 1;

  $(`.movieOption:nth-of-type(${nthOfTypeValue}) div`)
    .text(
      "✔️ Released: " +
      radMovieQuiz.finalFour[answerIndex].release_date.split("-")[0]
    )
...
radMovieQuiz.eventListener = function () {
  ...
  $(".movieOption input[type=image]").on("click", function (e) {
    ...
    if (radMovieQuiz.IsQuizOn) {
      ...
      radMovieQuiz.selectedMovie =
        radMovieQuiz.finalFour[radMovieQuiz.selectedMovieIndex - 1];
...
//load questions - from the locally stored array (so no round-trip to the API required)
radMovieQuiz.loadNextQuestion = () => {
...
//helper function to generate a few random numbers
